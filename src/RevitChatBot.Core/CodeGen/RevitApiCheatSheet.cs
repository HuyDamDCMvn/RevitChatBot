...
public static class RevitApiCheatSheet
{
    public static string GetCheatSheet() => """
        ...
        ## CONNECTOR TRAVERSAL (production pattern)
        ```
        ...
        foreach (Connector c in cm.Connectors)
        {
            ...
            var domain = c.Domain; // DomainHvac, DomainPiping, etc.
            double flow = c.Flow;
            if (c.IsConnected)
            {
                foreach (Connector other in c.AllRefs)
                {
                    if (other?.Owner is Element neighbor && neighbor.Id != elem.Id)
                    {
                        // neighbor is the connected element
                    }
                }
            }
        }
        ...
        var queue = new Queue<(Element, int)>();
        ...
        ## ELBOW / FITTING CREATION
        ```
        // Create elbow between two connectors
        ...
        ## ROUTING PREFERENCES
        ```
        // Check pipe routing preferences (elbow/junction rules)
        ...
        // Check duct routing preferences
